...
async function runContainers() {
  ...
  try {
    ...
    execSync(`docker-compose -f ${composePath} up -d redis`, { cwd: __dirname, stdio: 'inherit' });
    await waitForHealthy('redis');
    console.log(' Redis is healthy and ready');

    console.log(' Starting Minio via docker-compose...');
    execSync(`docker-compose -f ${composePath} up -d minio`, { cwd: __dirname, stdio: 'inherit' });
